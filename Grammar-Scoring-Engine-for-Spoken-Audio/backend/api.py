...
def get_services():
    ...
    global _transcription_service, _scorer
    ...
    return _transcription_service, _scorer
...
async def predict_grammar_score(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Get services
        transcription_service, scorer = get_services()
...
async def transcribe_audio(file: UploadFile = File(...)):
    ...
    try:
        ...
        transcription_service, _ = get_services()
...
async def score_text(text: str):
    ...
    Returns grammar score (0-5) with confidence
    """
    if not text.strip():
